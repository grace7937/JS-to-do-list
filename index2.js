...
function toggleAllTodoHandler() {
  const isAllCompleted = todos.every(todo => todo.status === TODO_STATUS.COMPLETED);
...
function deleteTodoHandler(todo) {
  todos = todos.filter(({id}) => todo.id !== id);
...
const getWriteInput = todo => {
  ...
  writeInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      todos = todos.map(_todo => _todo.id === todo.id
        ? {..._todo, text: writeInput.value}
        : _todo
      )
